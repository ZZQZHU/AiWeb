  ...
  <div class="container">
    <div class="mainbox">
      <div class="chatBox" v-for="(chat, index) in ChatlistData" :key="index">
        ...
        <div class="systemChat" v-else-if="chat.type == '0'">
          <div class="chatImg">
...
const saveChatRecord = async (data: any) => {
  const response = await fetch("https://backsys.aiblog.top/Kimi/chat/recordchat", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
const getChatRecord = async () => {
  const response = await fetch("https://backsys.aiblog.top/Kimi/chat/getchatrecord", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
//   const response = await fetch('https://backsys.aiblog.top/Kimi/chat/filelist', {
//     method: "POST",
//     headers: {
//       "Content-Type": "application/json",
//     },
...
const ChatCompletion = debounce(async () => {
  ...
  let userchatdata = {
    role: "user",
    content: searchData.value ? searchData.value : "整理文件的核心内容",
    type: "0",
  ...
  if (dayToken.value > 10000) {
    isAnswer.value = false;
    // 添加系统消息到聊天列表
    const systemChatData = {
      role: "system",
      content: "您好，今日对话次数已用完，请明天再来！",
      type: "0",
      uploadListsimg: [],
      uploadListsfile: [],
    };
  ...
  // 添加token记录
  let tokenparams = JSON.stringify({
    model: "moonshot-v1-8k",
    messages: KimiData,
    user_id: dataStore.getCookie("user_id")
  });
  const responseToken = await fetch('https://backsys.aiblog.top/Kimi/chat/estimatetoken', {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  const response = await fetch('https://backsys.aiblog.top/Kimi/chat/chatstream', {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
